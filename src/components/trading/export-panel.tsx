  ...
  if (!hasTrades) {
    return (
      ...
      >
        ...
        <h3 className="text-lg font-bold text-foreground mb-2">Exporter vos données</h3>
    ...
    >
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Téléchargez vos données en CSV ou générez un rapport PDF professionnel
        </p>
      </div>
      ...
      <div className="glass-card rounded-2xl p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-[11px] text-muted-foreground">
          ...
          <div className="hidden md:block">
            <p className="font-medium text-foreground mb-1">Données incluses</p>
            <ul className="space-y-0.5 ml-3 list-disc">
              <li>{trades.length} trades</li>
              <li>Du {trades[trades.length - 1]?.date ? new Date(trades[trades.length - 1].date).toLocaleDateString('fr-FR') : 'N/A'}</li>
              <li>Au {trades[0]?.date ? new Date(trades[0].date).toLocaleDateString('fr-FR') : 'N/A'}</li>
