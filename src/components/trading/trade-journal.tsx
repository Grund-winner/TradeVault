  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-white">Journal de Trading</h3>
          <p className="text-xs text-[#94a3b8] mt-0.5">{trades.length} trade{trades.length > 1 ? 's' : ''} enregistre{trades.length > 1 ? 's' : ''}</p>
        </div>
        <Button
          onClick={onAddClick}
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-white/[0.06] hover:bg-transparent">
                <TableHead className="text-[#94a3b8]"><SortHeaderCell label="Date" k="date" onSort={toggleSort} /></TableHead>
                <TableHead className="text-[#94a3b8]"><SortHeaderCell label="Instrument" k="instrument" onSort={toggleSort} /></TableHead>
                <TableHead className="text-[#94a3b8]"><SortHeaderCell label="Direction" k="direction" onSort={toggleSort} /></TableHead>
                <TableHead className="text-[#94a3b8] text-right">Entree</TableHead>
                <TableHead className="text-[#94a3b8] text-right">SL</TableHead>
                <TableHead className="text-[#94a3b8] text-right">TP</TableHead>
                <TableHead className="text-[#94a3b8] text-right"><SortHeaderCell label="P&L ($)" k="pnl" onSort={toggleSort} /></TableHead>
              ...
              <AnimatePresence mode="popLayout">
                {paginated.map((trade) => (
                  <motion.tr
                    ...
                    className="trade-row border-b border-white/[0.04] cursor-default"
                  >
                    ...
                    <TableCell className="text-sm text-[#94a3b8] text-right font-mono">{trade.entry}</TableCell>
                    <TableCell className="text-sm text-[#94a3b8] text-right font-mono">{trade.stopLoss}</TableCell>
                    <TableCell className="text-sm text-[#94a3b8] text-right font-mono">{trade.takeProfit}</TableCell>
                    ...
                    <TableCell>
                      <button
                        ...
                        className="p-1.5 rounded-lg text-[#94a3b8]/40 hover:text-[#ef4444] hover:bg-[#ef4444]/10 transition-all"
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-3 border-t border-white/[0.06]">
            ...
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="sm" onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} className="h-8 px-2 text-[#94a3b8] hover:text-white">
                <ChevronLeft className="h-4 w-4" />
              </Button>
              {Array.from({ length: totalPages }, (_, i) => (
                <button key={i} onClick={() => setPage(i)} className={`w-8 h-8 rounded-lg text-xs font-medium transition-all ${page === i ? 'bg-[#ff6b2b] text-white' : 'text-[#94a3b8] hover:bg-white/5 hover:text-white'}`}>
                  {i + 1}
                </button>
              ))}
              <Button variant="ghost" size="sm" onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page === totalPages - 1} className="h-8 px-2 text-[#94a3b8] hover:text-white">
